...
var
	...
	//先创建一个空对象，空数组，然后就可以调用对象和数组中继承的方法了。
	...
	// 	})
	// }
	//all表示全部匹配的，而letter只是指匹配小括号的那一部分
			...
			if ( match && (match[1] || !context) ) {
				...
				// HANDLE: $(#id)
					...
					if ( elem && elem.parentNode ) {
						// Inject the element directly into the jQuery object将其变为一个jQuery对象
						this.length = 1;
						this[0] = elem;
					}
...
jQuery.extend = jQuery.fn.extend = function() {
	var options, name, src, copy, copyIsArray, clone,
		target = arguments[0] || {},  //有参数的话，target就取第一个
		i = 1,	//其实它在这里的时候也是在跳过target
		length = arguments.length,
		deep = false;  //默认下为false
	...
	// 如果传入的第一个参数是 字符串或者其他函数，就会变成一个空对象
	...
	//因为前面一直把target跳过去，所以如果相等的话，只有一个可能，
	//那就是长度为 1
		target = this;
		--i;
	}
...
jQuery.extend({
	...
	// \xA0 是全角空格
	//之前js并不支持trim，所以还是需要判断的，现在已经支持了
	trim: function( text ) {
